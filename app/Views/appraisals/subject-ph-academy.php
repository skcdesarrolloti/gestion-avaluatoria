<?php
/** @var array<int, array{src:string, quote:string, use:string, report:string}> $phAcademy */
    ...
    <section class="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm leading-6 text-indigo-950">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-xs text-indigo-800">Cita clave y aplicación práctica, sin convertir la pantalla en una biblioteca completa.</p>
            </div>
            ...
            <table class="w-full min-w-[72rem] text-left text-xs leading-5">
                ...
                <tbody class="divide-y divide-indigo-50 text-slate-700">
                    <?php foreach ($phAcademy as $row): ?>
                        <tr><td class="p-2 font-bold text-indigo-900"><?= e($row['src']) ?></td><td class="p-2 italic text-slate-800"><?= e($row['quote']) ?></td><td class="p-2"><?= e($row['use']) ?></td><td class="p-2"><?= e($row['report']) ?></td></tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
