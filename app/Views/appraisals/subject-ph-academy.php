<?php
/** @var array<int, array{src:string, says:string, use:string, report:string}> $phAcademy */
    ...
    <section class="rounded-xl border border-indigo-100 bg-indigo-50 p-4 text-sm leading-6 text-indigo-950">
        <div class="flex flex-wrap items-start justify-between gap-3">
            <div>
                ...
                <p class="mt-1 text-xs text-indigo-800">Qué dice cada referencia y cómo se aplica sin copiar la norma completa al informe.</p>
            </div>
        ...
        <div class="mt-3 overflow-x-auto rounded-lg border border-indigo-100 bg-white">
            <table class="w-full min-w-[72rem] text-left text-xs leading-5">
                <thead class="bg-indigo-100/70 uppercase text-indigo-800">
                    <tr><th class="p-2">Referencia</th><th class="p-2">Qué dice la norma</th><th class="p-2">Cómo se aplica aquí</th><th class="p-2">Qué pasa al entregable</th></tr>
                </thead>
                <tbody class="divide-y divide-indigo-50 text-slate-700">
                    <?php foreach ($phAcademy as $row): ?>
                        <tr><td class="p-2 font-bold text-indigo-900"><?= e($row['src']) ?></td><td class="p-2"><?= e($row['says']) ?></td><td class="p-2"><?= e($row['use']) ?></td><td class="p-2"><?= e($row['report']) ?></td></tr>
                    <?php endforeach; ?>
                </tbody>
            </table>
        </div>
