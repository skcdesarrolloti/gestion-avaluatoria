...
$scoreJson = json_encode($scoreStateAll, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR);
$evidenceJson = json_encode($evidenceStateAll, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR);
?>
<form class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8" method="post" action="<?= e(url($subjectActionBase . '/obsolescencias')) ?>" data-module-autosave data-save-in-place data-autosave-endpoint="<?= e(url($subjectActionBase . '/obsolescencias/autoguardar')) ?>" x-data='{activeObs:"fisica", allScores: <?= e($scoreJson) ?>, allEvidence: <?= e($evidenceJson) ?>, value(v){return ["0","1","2","3"].includes(v)?Number(v):null}, values(group){return Object.values(this.allScores[group]||{})}, applicable(group){return this.values(group).filter(v=>this.value(v)!==null).length}, sum(group){return this.values(group).reduce((t,v)=>t+(this.value(v)??0),0)}, max(group){return this.applicable(group)*3}, ieo(group){return this.max(group)?(this.sum(group)/this.max(group))*100:0}, levelFrom(ieo){return ieo===0?"Sin hallazgos":(ieo<=33.33?"Leve":(ieo<=66.67?"Relevante":"Crítica"))}, groupLevel(group){return this.levelFrom(this.ieo(group))}, groupCalc(group){return this.applicable(group)?`${this.sum(group)} de ${this.max(group)} puntos posibles = ${this.ieo(group).toFixed(1).replace(".",",")} %`:"Sin factores revisados"}, missing(group){return Object.entries(this.allScores[group]||{}).filter(([k,v])=>["2","3"].includes(v)&&(((this.allEvidence[group]||{})[k]||"").trim()==="")).length}, state(group){return this.missing(group)>0?"warn":(this.applicable(group)>0?"ok":"optional")}, stateText(group){return this.state(group)==="warn"?"Revisar":(this.state(group)==="ok"?"Completo":"Opcional")}, stateClass(group){return this.state(group)==="warn"?"bg-amber-100 text-amber-800":(this.state(group)==="ok"?"bg-emerald-100 text-emerald-800":"bg-slate-100 text-slate-700")}, actionText(group){return this.missing(group)>0?"Agregar soporte breve en hallazgos relevantes o críticos.":(this.applicable(group)>0?"Listo para lectura.":"Pendiente; marque 0 si ya revisó y no encontró hallazgos.")}, globalValues(){return Object.values(this.allScores).flatMap(g=>Object.values(g))}, globalApplicable(){return this.globalValues().filter(v=>this.value(v)!==null).length}, globalSum(){return this.globalValues().reduce((t,v)=>t+(this.value(v)??0),0)}, globalIeo(){return this.globalApplicable()?this.globalSum()/(this.globalApplicable()*3)*100:0}, globalLabel(){return `IEO diagnóstico global: ${this.globalIeo().toFixed(1).replace(".",",")} % · ${this.levelFrom(this.globalIeo())}`}}'>
    <?= csrf_field() ?>
    ...
    <div class="mt-5 rounded-xl border border-slate-200 bg-white p-4 text-sm leading-6">
        ...
        <div class="mt-3 overflow-x-auto"><table class="w-full min-w-[54rem] text-left text-sm"><thead class="text-xs uppercase text-slate-500"><tr><th class="py-2 pr-3">Campo</th><th class="py-2 pr-3">Lectura</th><th class="py-2 pr-3">Estado</th><th class="py-2">Qué hacer</th></tr></thead><tbody class="divide-y divide-slate-100">
            ...
            <?php foreach ($groups as $groupKey => [$code, $title]): ?>
                <tr><td class="py-2 pr-3 font-semibold"><?= e($title) ?></td><td class="py-2 pr-3" x-text="'<?= e($code) ?> · ' + groupCalc('<?= e((string) $groupKey) ?>').replace(' puntos posibles =', ' =') + ' · ' + groupLevel('<?= e((string) $groupKey) ?>')"><?= e($code . ' · ' . number_format((float) $metrics[(string) $groupKey]['ieo'], 1, ',', '.') . ' % · ' . $metrics[(string) $groupKey]['level']) ?></td><td class="py-2 pr-3"><span class="rounded-full px-3 py-1 text-xs font-bold" :class="stateClass('<?= e((string) $groupKey) ?>')" x-text="stateText('<?= e((string) $groupKey) ?>')"><?= e($metrics[(string) $groupKey]['missing'] > 0 ? 'Revisar' : ($metrics[(string) $groupKey]['applicable'] > 0 ? 'Completo' : 'Opcional')) ?></span></td><td class="py-2 text-slate-600" x-text="actionText('<?= e((string) $groupKey) ?>')"><?= e($metrics[(string) $groupKey]['missing'] > 0 ? 'Agregar soporte breve en hallazgos relevantes o críticos.' : ($metrics[(string) $groupKey]['applicable'] > 0 ? 'Listo para lectura.' : 'Pendiente; marque 0 si ya revisó y no encontró hallazgos.')) ?></td></tr>
            <?php endforeach; ?>
