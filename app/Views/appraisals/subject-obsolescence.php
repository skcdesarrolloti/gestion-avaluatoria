...
$metrics = []; $scoreStateAll = [];
foreach ($groups as $groupKey => [, , , , $items]) {
    $metrics[$groupKey] = $metric((string) $groupKey, $items);
    $scoreStateAll[$groupKey] = [];
    foreach ($items as $itemKey => $_label) $scoreStateAll[$groupKey][(string) $itemKey] = $factor((string) $groupKey, (string) $itemKey, 'score');
}
    ...
    <div class="mt-5 rounded-xl border border-slate-200 bg-white p-4 text-sm leading-6">
        ...
        <div class="mt-3 overflow-x-auto"><table class="w-full min-w-[54rem] text-left text-sm"><thead class="text-xs uppercase text-slate-500"><tr><th class="py-2 pr-3">Campo</th><th class="py-2 pr-3">Lectura</th><th class="py-2 pr-3">Estado</th><th class="py-2">Qué hacer</th></tr></thead><tbody class="divide-y divide-slate-100">
            ...
            <?php foreach ($groups as $groupKey => [$code, $title]): $m = $metrics[(string) $groupKey]; $state = $m['missing'] > 0 ? 'warn' : ($m['applicable'] > 0 ? 'ok' : 'optional'); ?>
                <tr><td class="py-2 pr-3 font-semibold"><?= e($title) ?></td><td class="py-2 pr-3"><?= e($code . ' · ' . number_format((float) $m['ieo'], 1, ',', '.') . ' % · ' . $m['level']) ?></td><td class="py-2 pr-3"><?= $pill($state) ?></td><td class="py-2 text-slate-600"><?= e($m['missing'] > 0 ? 'Agregar soporte breve en hallazgos relevantes o críticos.' : ($m['applicable'] > 0 ? 'Listo para lectura.' : 'Pendiente; marque 0 si ya revisó y no encontró hallazgos.')) ?></td></tr>
            <?php endforeach; ?>
