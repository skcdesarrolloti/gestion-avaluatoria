...
<div class="mt-5 rounded-xl border border-slate-200 p-5" data-attribute-unit data-unit-id="<?= e($unitId) ?>"
    ...
    <?php if (count($catalog) === 1): ?>
        <p class="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm leading-6 text-amber-900">
            Solo aparece Base común porque esta unidad no tiene tipo de inmueble definido. Ve a 3.1 Ficha básica del sujeto y selecciona casa, apartamento, lote, local, oficina, bodega u otro tipo para cargar su lista completa de atributos.
        </p>
    <?php endif; ?>
    <div class="mt-4 flex flex-wrap items-center justify-between gap-3 rounded-xl border border-amber-100 bg-amber-50 p-4 text-sm text-amber-950">
        <span><strong>Selección valuatoria:</strong> revisa todos los atributos del tipo de inmueble y marca solo los que incidan en valor. El perito decide la selección; si supera 6, el sistema avisará que el máximo técnico sugerido es 6.</span>
