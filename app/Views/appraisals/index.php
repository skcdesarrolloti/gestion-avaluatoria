...
<nav aria-label="Paginación" class="mt-7 flex gap-3">
    <?php if ($page > 1): ?><a class="btn-secondary" href="<?= e(url('?page=' . ($page - 1) . ($searchValue !== '' ? '&q=' . urlencode($searchValue) : ''))) ?>">Anterior</a><?php endif; ?>
    <?php if ($hasNext): ?><a class="btn-secondary" href="<?= e(url('?page=' . ($page + 1) . ($searchValue !== '' ? '&q=' . urlencode($searchValue) : ''))) ?>">Siguiente</a><?php endif; ?>
</nav>
