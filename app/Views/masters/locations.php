<section id="maestros-geograficos" class="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
    <div class="flex flex-wrap items-start justify-between gap-4">
        <div>
            ...
            <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                Crea primero departamentos, luego ciudades o municipios, y finalmente barrios, veredas o sectores.
        ...
        <span class="rounded-full bg-teal-50 px-3 py-1 text-sm font-semibold text-teal-800">
            <?= e(count($departments)) ?> depto(s) · <?= e(count($cities)) ?> ciudad(es)
        </span>
    </div>
    ...
    <div class="mt-6 grid gap-4 lg:grid-cols-3">
        <?php foreach ([['Departamentos', $departments], ['Ciudades', $cities], ['Barrios / sectores', $neighborhoods]] as [$title, $rows]): ?>
                ...
                <?php else: ?>
                    <ul class="mt-3 space-y-1 text-sm text-slate-700">
                        <?php foreach (array_slice($rows, 0, 8) as $row): ?>
                            <li><?= e(($row['department_name'] ?? '') ? ($row['department_name'] . ' · ') : '') ?><?= e(($row['city_name'] ?? '') ? ($row['city_name'] . ' · ') : '') ?><?= e($row['name']) ?></li>
                        <?php endforeach; ?>
                    </ul>
                <?php endif; ?>
            </div>
