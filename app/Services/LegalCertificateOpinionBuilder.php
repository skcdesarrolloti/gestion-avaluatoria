...
final class LegalCertificateOpinionBuilder
{
    public function build(array $data, array $tradition, array $debts, array $limits, array $measures, array $ph, array $alerts): array
    {
        ...
        $diagnosis = $this->diagnosis($level, $activeDebts, $activeLimits, $activeMeasures, $folioIssue);
        return [
            'level' => $level,
            'classification' => $classification,
            'conclusion' => $diagnosis,
            'integrated' => $this->integrated($data, $tradition, $activeDebts, $activeLimits, $activeMeasures, $ph, $alerts, $diagnosis),
        ];
    }

    private function diagnosis(string $level, array $debts, array $limits, array $measures, bool $folioIssue): string
    {
        if ($level === 'Normal') {
            return 'Situación jurídica preliminar aparentemente saneada para fines del avalúo: en la lectura automática no se observan gravámenes, limitaciones al dominio ni medidas cautelares vigentes concluyentes. La compra o venta podría avanzar documentalmente, siempre que el analista confirme el certificado completo, identidad del titular, paz y salvo, tradición inmediata y soportes urbanísticos.';
        }
        $issues = [];
        ...
        if ($level === 'Crítico') {
            return 'Situación jurídica preliminar no saneada para cierre sin revisión especializada: se observan ' . $issueText . '. No se recomienda perfeccionar compra, venta, hipoteca o transferencia hasta que un abogado de títulos verifique vigencia, cancelaciones, levantamientos, partes intervinientes y posibilidad real de disposición del inmueble.';
        }
        return 'Situación jurídica preliminar comercializable con condiciones: se observan ' . $issueText . '. La operación puede estudiarse, pero antes de prometer, comprar o vender debe verificarse que las cargas estén canceladas o sean aceptables para las partes y que no impidan la libre disposición.';
    }

    private function integrated(array $data, array $tradition, array $debts, array $limits, array $measures, array $ph, array $alerts, string $diagnosis): string
    {
        $parts = [];
