...
final class AppraisalMidasFileStorage
{
    ...
    public static function storeUploaded(string $tmpName, string $destination): int
    {
        ...
        if (!$moved || !is_file($destination)) throw new \RuntimeException('El soporte MIDAS no quedó guardado.');
    ...
    private static function ensure(): void
    {
        ...
        if (!is_dir($dir) && !mkdir($dir, 0775, true) && !is_dir($dir)) {
            throw new \RuntimeException('No se pudo preparar el almacenamiento MIDAS.');
        }
    }
